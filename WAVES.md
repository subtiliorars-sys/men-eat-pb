# WAVES.md — Men Eat Peanut Butter

Pending waves for the fleet worker. One PR per wave; run `npm run verify` before open.

## MEP-W1 — Vertical slice (DONE locally)
- [x] Sim engine + unit tests
- [x] PicnicScene: four men, blobs, modifiers, Frenzy, run end
- [x] README + repo scaffold

## MEP-W2 — Juice pass (DONE)
- [x] Chomp squash animation polish
- [x] Web Audio chomp + Frenzy pitch (optional mute)
- [x] Miss splat visual on table
- [x] Screen border pulse during Frenzy
- [x] Visual jar fill depletion + miss/sticky HUD

## MEP-W3 — Meta stub (DONE)
- [x] Persist Crust Credits (localStorage)
- [x] Upgrade shop: Deeper Jar I, Golden Spoon
- [x] Run summary shows unlock hints

## MEP-W4 — Table Events (DONE)
- [x] Two mid-run events (*Ant Invasion*, *Mom Says Share*)
- [x] Event picker at 50% jar depletion

## MEP-W5 — QA gate
- [x] `docs/QA_BROWSER.md` + dev QA bridge (DrivingMeNuts pattern)
- [x] Community playtest feedback intake + volunteer review workflow
- [x] Dynamic feedback API contract + separate volunteer admin scaffold
- [x] Optional Worker/D1 feedback backend scaffold
- [x] game-provost tutorial audit — first-run tutorial + replay from start screen (`docs/QA_TUTORIAL_AUDIT.md`)

## Ethics

No dark patterns. No forced wait timers. See MeniscusMaximus YES_MAN_GDD P2 alignment.
